...
object FingerprintSpoofer {
    ...
    fun getSpoofingScript(profile: BrowserProfile): String {
        ...
        // 1. Spoof Navigator (User-Agent, Platform, Languages)
        val ua = getProfileUserAgent(profile)
        val platform = getProfilePlatform(profile)
        val languages = getProfileLanguages(profile)
        
        val needsUaOver = (ua != "Default" && ua.isNotEmpty())
        val needsPlatformOver = (profile.spoofedPlatform != "Default" && profile.spoofedPlatform.isNotEmpty())
        val needsLanguagesOver = (profile.spoofedLanguages != "Default" && profile.spoofedLanguages.isNotEmpty())

        if (needsUaOver || needsPlatformOver || needsLanguagesOver) {
            hasModifications = true
            sb.append("\n                try {")
            if (needsUaOver) {
                sb.append("\n                    Object.defineProperty(navigator, 'userAgent', { get: function() { return \"$ua\"; } });")
            }
            if (needsPlatformOver) {
                sb.append("\n                    Object.defineProperty(navigator, 'platform', { get: function() { return \"$platform\"; } });")
            }
            if (needsLanguagesOver) {
                sb.append("\n                    Object.defineProperty(navigator, 'languages', { get: function() { return [$languages]; } });")
            }
            
            // Add userAgentData overrides if userAgent is overridden
            if (needsUaOver) {
            ...
            sb.append("\n                } catch (e) { console.error(\"Navigator spoofing failed: \", e); }")
        }

        // 2. Spoof Timezone
        if (profile.spoofedTimezone != "Default" && profile.spoofedTimezone.isNotEmpty()) {
            hasModifications = true
        ...
        // 3. Spoof Geolocation
        if (profile.spoofedGeolocation != "Default" && profile.spoofedGeolocation.isNotEmpty()) {
        ...
        // 4. WebRTC Block/Leak protection - protect local IP leak
        sb.append("""
            try {
        ...
        // 5. Canvas noise injection if enabled
        ...
        // 6. WebGL GPU spoofing if enabled
        if (profile.webGlSpoofEnabled) {
            hasModifications = true
        ...
        // If no optional modifiers are enabled, do not load ANY third-party JS, keeping client pristine
        if (!hasModifications) {
            return ""
        }
    ...
    fun getProfileUserAgent(profile: BrowserProfile): String {
        return when (profile.userAgentType) {
            "Chrome Mobile" -> "Mozilla/5.0 (Linux; Android 14; Pixel 8 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Mobile Safari/537.36"
            ...
            "Firefox Mobile" -> "Mozilla/5.0 (Android 14; Mobile; rv:124.0) Gecko/124.0 Firefox/124.0"
            "Edge Mobile" -> "Mozilla/5.0 (Linux; Android 14; Pixel 8 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Mobile Safari/537.36 EdgA/124.0.0.0"
            ...
            "Custom" -> if (profile.customUserAgent.isNotEmpty()) profile.customUserAgent else "Default"
            else -> "Default"
        }
    }
    ...
    fun getProfilePlatform(profile: BrowserProfile): String {
        ...
        return when (profile.userAgentType) {
            ...
            "Chrome Mobile", "Firefox Mobile", "Edge Mobile" -> "Linux armv8l"
            else -> "Linux armv8l"
        }
    }
    ...
    private fun getProfileWebGL(profile: BrowserProfile): Pair<String, String> {
        return when (profile.userAgentType) {
            ...
            "Chrome Mobile", "Edge Mobile" -> Pair("Google Inc. (Qualcomm)", "Adreno (TM) 730")
            "Firefox Mobile" -> Pair("Google Inc. (Qualcomm)", "Adreno (TM) 730")
